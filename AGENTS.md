...
- **Every study must update the planning surfaces before it is considered complete.** Add or update the actionable findings in `docs/prioritized-backlog-2026-07-15.md`, rebuild and verify the local-only `/backlog/` HTML view so the feature grouping and item status reflect the Markdown source, and update the relevant pipeline, operations, README, or findings documentation. Keep the study's evidence, backlog IDs, and documentation links consistent.
