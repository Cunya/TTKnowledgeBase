...
**Status:** Controlled expansion and safety-gated automated queue triage continued through 2026-07-17. The automatic processor accepts only high-confidence matches to existing canonical concepts, validates the cited span, and defers new or ambiguous concepts. After the latest pass, all 1,647 queue items have explicit decisions: 1,303 accepted, 343 deferred, and 1 rejected; no undecided items remain. The published corpus validates at 73 concepts, 182 videos, and 2,056 evidence moments. The local Recent page reads a script-generated normalized-video snapshot so newly ingested lessons appear immediately as awaiting extraction/review; `ingest` and `publish` both refresh that snapshot automatically.
...
**Done when:** every pending item is accepted, rejected, or explicitly deferred with a reason, and one-concept videos have been checked against their candidate set. **Evidence:** all 1,647 queue items have explicit decisions; the latest validation reports 73 concepts, 182 videos, and 2,056 evidence moments. New/ambiguous candidates remain deferred rather than being auto-created as public concepts.
...
**Status:** In progress — Phase 1 caption-only baseline and Phase 2 review-worksheet tooling were implemented (2026-07-16). The first 24-item review pass recorded 9 safe caption-context merges, 4 meaningful short-window keeps, and 11 conservative deferrals; the deferred cases still need source/audio inspection and measured reduction remains open. The `report-boundaries` command measured 1,826/1,826 local-caption-backed moments: 16.4% start mid-sentence, 17.5% end mid-sentence, 3.5% are under four seconds, and 31.9% require context review. `process-pending` defers newly flagged candidates with the boundary flags instead of silently accepting them. `prepare-boundary-review` exported the deterministic sample from the 582 flagged moments, and `validate-boundary-review` checks the recorded decisions without applying them. A 1.2-second UI-only spoken-playback tail now cushions caption timing without changing transcript citations; the remaining work is audio/visual inspection of deferred windows. Baseline: `docs/moment-boundary-report-table-tennis-2026-07-16.md`.
